# Note de synthèse — Activation et conversion utilisateur e-commerce

## Contexte

Cette analyse porte sur le parcours utilisateur d’un site e-commerce à partir d’événements GA4 stockés dans BigQuery.

Période analysée : **2020-11-01 → 2021-01-31**

Population principale : **257 314 nouveaux utilisateurs observés**, définis comme les utilisateurs ayant un événement `first_visit` dans la période.

---

## Objectif

Identifier où les nouveaux utilisateurs décrochent dans le parcours produit, et mesurer le lien entre activation, conversion et rétention.

Définition de l’activation :

> un utilisateur est activé s’il réalise un `add_to_cart` dans les 7 jours suivant son `first_visit`.
...
Sur **257 314 nouveaux utilisateurs**, seuls **10 280** atteignent l’étape d’activation.

**Taux d’activation : 4,0 %**

Cela signifie que la majorité des nouveaux utilisateurs ne génère pas de signal fort d’intention d’achat.
...
### 3. Le principal décrochage se situe au début du funnel
...
### 5. La friction semble globale

Les taux d’activation et d’achat sont proches entre desktop, mobile et tablet.
...
Il n’y a pas de problème évident limité à un device spécifique.
...
Le principal enjeu est plus en amont :

> trop peu de nouveaux utilisateurs consultent une fiche produit, puis ajoutent un produit au panier.

L’activation est le levier prioritaire, car elle est fortement associée à la conversion et à la rétention.
...
- Les données couvrent **92 jours**, ce qui limite l’analyse de rétention long terme.
...
Le levier produit prioritaire n’est pas l’optimisation isolée du checkout, mais l’amélioration du parcours initial.
